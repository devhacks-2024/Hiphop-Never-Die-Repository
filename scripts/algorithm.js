...
function shortestPath(arr, floor, row, col, x1, y1, z1, x2, y2, z2) {
  ...
  while (!(goal[0] === x1 && goal[1] === y1 && goal[2] === z1)) {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (
        xNeighbor >= 0 && xNeighbor < col &&
        yNeighbor >= 0 && yNeighbor < row &&
        zNeighbor >= 0 && zNeighbor < floor &&
    ...
    console.log(`Move to: (${goal[0]}, ${goal[1]}, ${goal[2]})`);
  }

  return dist[z2][y2][x2];
}
...
shortestPath(arr, LAYER_COUNT, ROW_COUNT, COL_COUNT, 1, 12, 0, 5, 23, 1);
